...
{	
	m_pConnection.CreateInstance(__uuidof(AdoNS::Connection));	// Open the Connection
}
...
*/
	...
	if(m_nDBtype ==1)
	{
		...
		//strConn.Format(_T("Driver={MySQL ODBC 8.0 Unicode Driver};Server=%s;Database=%s;UID=%s;PWD=%s")
		strConn.Format(_T("Driver={MySQL ODBC 9.2 Unicode Driver};Server=%s;Database=%s;UID=%s;PWD=%s")
			, m_serverIP
			, m_catalog
			, m_userID
